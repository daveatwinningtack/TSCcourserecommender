...
_POLAR_SPEED = [
    [1.50, 1.99, 2.49, 2.99, 3.49, 3.74, 3.74],  # 52°
    [1.56, 2.09, 2.61, 3.13, 3.65, 3.91, 3.91],  # 60°
    [1.70, 2.26, 2.83, 3.39, 3.96, 4.24, 4.24],  # 75°
    [2.35, 3.13, 3.91, 4.69, 5.48, 5.87, 5.87],  # 90°
    [2.46, 3.28, 4.10, 4.93, 5.75, 6.16, 6.16],  # 110°
    [2.52, 3.36, 4.20, 5.04, 5.88, 6.30, 6.30],  # 120°
    [2.61, 3.48, 4.35, 5.22, 6.08, 6.52, 6.52],  # 135°
    [1.96, 2.61, 3.26, 3.91, 4.56, 4.89, 4.89],  # 150°
]
...
with st.expander("Mark names and aliases"):
    st.table(pd.DataFrame(MARK_DATA))    dlam = lam2 - lam1
    x = sin(dlam) * cos(phi2)
    y = cos(phi1)*sin(phi2) - sin(phi1)*cos(phi2)*cos(dlam)
